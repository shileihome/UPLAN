 ...
 * Description:
 */
public abstract class BaseRecyclerActivity<P extends IPresenter> extends BaseActivity<P> {
    ...
    private void initHeaderFooter() {
        if (getHeaderLayout() != NO_LAYOUT) {
            View header = LayoutInflater.from(this).inflate(getHeaderLayout(), mRv, false);
            onInitHeader(header);
        ...
        if (getFooterLayout() != NO_LAYOUT) {
            View footer = LayoutInflater.from(this).inflate(getFooterLayout(), mRv, false);
            onInitFooter(footer);
